...
typedef struct DefaultConfigMSG
{
...
typedef struct VProgramListMSG {

	WORD Opcode;
	unsigned short mode;
	unsigned char destinationIP[4];
	unsigned short destinationUDP;
	VProgramEntry ProgramList[NUMBER_OF_VPROGRAMS];
	WORD Random;		
	BYTE Signature[20];				
...
typedef struct GetProgramInfoMSG {
...
typedef struct ProgramInfo {
	...
	LanguageDescriptor langList[NUMBER_OF_AUDIO_PIDS_PER_PROGRAM];

} ProgramInfo;


typedef struct ProgramInfoReplyMSG {

	ProgramInfo Info[NUMBER_OF_VPROGRAMS];
...
//	TCHAR temp[16];
	...
	fd_set readfs;
	int result;
	VProgramListMSG programRequest;
    BYTE Buffer2[sizeof(VProgramListMSG)];
	...
	try {
		...
		digitalsignature( &m_sec, (uint8_t *)(&programRequest), sizeof( VProgramListMSG ) - 20 );
        ...
        for ( i = 0; (!success) && ( i < 4 ); i++) {
            ...
            if ( send( login_socket, (const char *)(&programRequest), sizeof( VProgramListMSG ), 0 ) != sizeof( VProgramListMSG ) ) {
...
#ifdef WINDOWS
  		    	        if( (temp1 = recv( login_socket, (char *)Buffer2, sizeof(VProgramListMSG), 0 ) ) == SOCKET_ERROR ) {
...
#else
   		    	        if( (temp1 = recv( login_socket, (char *)Buffer2, sizeof(VProgramListMSG), 0 ) ) == -1 ) {
							last_error_code = N_ERROR_PID_RECEIVE;
#endif
    		    	        ...
    		    	        if (temp1 == sizeof(VProgramListMSG)) {
...
//	char temp[10];
	...
	if ( dirty ) {
		...
		try {
			...
			VProgramListMSG vprogramConfig;

			memset( (void *)&vprogramConfig, 0, sizeof( VProgramListMSG ) );
			...
			digitalsignature( &m_sec, (uint8_t *)(&vprogramConfig), sizeof( VProgramListMSG ) - 20 );
...
{
	DefaultConfigMSG factoryResetMsg;
	...
	digitalsignature( &m_sec, (uint8_t *)(&factoryResetMsg), sizeof( DefaultConfigMSG ) - 20 );

	memcpy( factoryResetMsg.Signature, m_sec.digest, 20 );

	return sendConfig( I_DEFAULT_CONFIG_PORT, (unsigned char *)&factoryResetMsg, sizeof( DefaultConfigMSG ) );
}
...
{
	bool success = false;
    VProgramListMSG VProgramListRequest;
	int i;
	unsigned char receive_buffer[1500];
	VProgramListMSG *VProgramListReply = (VProgramListMSG *)receive_buffer;
	char temp[20];
	...
	if ( getConfig( I_VIDEO_PROGRAM_PORT, (unsigned char *)&VProgramListRequest, receive_buffer, sizeof( VProgramListRequest ), sizeof( VProgramListMSG ) ) ) {
...
{
	bool success = false;
	GetProgramInfoMSG ProgramInfoRequest;
	unsigned char receive_buffer[1500];
	ProgramInfoReplyMSG *ProgramInfoReply = (ProgramInfoReplyMSG *)receive_buffer;
	int i;
	int index;
	int audio_index;
	char temp[20];
	...
	if ( getConfig( I_PROGRAM_INFO_PORT, (unsigned char *)&ProgramInfoRequest, receive_buffer, sizeof( ProgramInfoRequest ), sizeof( ProgramInfoReplyMSG ) ) ) {
...
{
//	bool success = false;
    GetPSIMSG PSIRequest;
//	int i;
	unsigned char receive_buffer[1500];
	SITable *siTable = (SITable *)receive_buffer;
