...
//	printf("GetDeviceType returned %d", Errcode);
	do{
		...
		switch(i){
			case 0:	if((Errcode == S75_JLEO)||(Errcode==S75CA))
            			result = logon(hdev);
					else
...
//	printf("\tIf you would like to use the Default value for a parameter, \n Hit Enter instead of inputting a value");
	...
	if( ( devType==S75D_V3 ) || ( devType==A75) || (devType==S75CA) || (devType==A75PesMaker) || (devType==PesMaker) || (devType==S75_JLEO) ) {
...
int userconfigureRF(S75_Handle hdev)
{
	...
	if ( (devType==S75D_V21) || (devType==S75D_V3) || (devType==PesMaker) ) {
		...
		if(result=(configureRF(hdev, (LPBYTE) &A75RFSet))== S75_OK)
			...
			printf("\n\n RF configuration Failed \n");
	} else if ( ( devType==S75_JLEO ) || ( devType == S75CA) ) {
...
*/
		...
		memset( (void *)(&statusS75CA), 0, sizeof( statusS75CA ) );
		...
		if((result=getStatus(hdev, (LPBYTE)&statusS75CA)) != S75_OK){
			printf("\n Failed to get status messages from the device\n");
			return result;
		}
		if ( k == 0 || k == 1 ) {
			printf("\n Unit Identification\n---------------------------------------------------");
			printf("\n Device Firmware\tVer %d, Rev. %d", statusS75CA.FirmWareVer,statusS75CA.FirmWareRev );
			printf("\n RF Firmware Code\t0x%X", statusS75CA.RF_FIRMWARE );
			printf("\n Unit ID Code\t\t");
			for(i=0;i<32;i++) printf("%0.2X ",statusS75CA.UID[i]);
			printf("\n" );
		}
		if ( k == 0 || k == 2 ) {
			printf("\n Network Configuration\n--------------------------------------------------\n" );
			...
			printf("\n Destination IP address\t\t");PrintIP(statusS75CA.UniCastStatusDestIP);
			printf("\n Destination UDP Port\t\t0x%X", statusS75CA.UniCastStatusDestUDP);
			printf("\n Broadcast UDP Port\t\t0x%X", statusS75CA.BroadcastStatusPort);
			printf("\n Receiver MAC Address\t\t"); PrintMAC(statusS75CA.ReceiverMAC);
//			printf("\n Receiver DVB MAC Address:  ");PrintMAC(statusS75CA.DVB_MACADDRESS);
			if(statusS75CA.IGMP_Enable > 0) printf("\n IGMP Filter\t\t\tON ");
			else printf("\n IGMP Filter\t\t\tOFF ");
			printf("\n\n Network Statistics\n--------------------------------------------------\n" );
			printf("\n Ethernet TX\t\t%d", statusS75CA.EthernetTransmit );
			printf("\n Ethernet RX\t\t%d", statusS75CA.EthernetReceive );
			printf("\n Ethernet Drops\t\t%d", statusS75CA.EthernetPacketDropped );
			printf("\n Ethernet TX Errors\t%d", statusS75CA.EthernetTransmitError );
			printf("\n Ethernet RX Errors\t%d", statusS75CA.EthernetReceiveError );
			printf("\n\n DVB Statistics\n--------------------------------------------------\n" );
			printf("\n Front End Lock Loss\t%d", statusS75CA.FrontEndLockLoss );
			printf("\n Data Sync Loss\t\t%d", statusS75CA.DataSyncLoss );
			printf("\n DVB Accepted\t\t%d", statusS75CA.DVBAccepted );
			printf("\n Total DVB Accepted\t%u %u", statusS75CA.TotalDVBPacketsAccepted[0], statusS75CA.TotalDVBPacketsAccepted[1] );
			printf("\n Total DVB RX Error\t%u %u", statusS75CA.TotalDVBPacketsRXInError[0], statusS75CA.TotalDVBPacketsRXInError[1] );
			printf("\n Total DVB Eth Out\t%u %u", statusS75CA.TotalEthernetPacketsOut[0], statusS75CA.TotalEthernetPacketsOut[1] );
			printf("\n Total Uncorr TS\t%u %u\n", statusS75CA.TotalUncorrectableTSPackets[0], statusS75CA.TotalUncorrectableTSPackets[1] );
		}
		if ( k == 0 || k == 3 ) {
			printf( "\n RF Status\n ---------------------------------------\n" );
			printf("\n Symbol Rate\t\t%f Msps", statusS75CA.SymbolRate );
			printf("\n Frequency\t\t%f", statusS75CA.Frequency );
			printf("\n LO Frequency\t\t%d", statusS75CA.LocalOscFrequency );
			printf("\n VBER\t\t\t%-0#4.4e", statusS75CA.VBER );
			printf("\n Viterbi Rate\t\t%d", statusS75CA.ViterbiRate );
			if(statusS75CA.Power != 0){
				printf("\n LNB Power\t\tON" );
				printf("\n Polarization\t\t%d", statusS75CA.Polarity );
				printf("\n Band\t\t\t%d", statusS75CA.Band );
				printf("\n Long Line\t\t%d", statusS75CA.Long_Line_Comp );
				printf("\n Hi Voltage Mode\t%d", statusS75CA.Hi_VoltageMode);
			/*	if(statusS75CA.LNBFault == 1){
				printf("\n LNB Fault ... \a\a");
			 } */
			}
			...
			printf("\n Signal Strength\t%d", statusS75CA.SignalStrength );
			printf("\n DemodGain\t\t%d", statusS75CA.DemodulatorGain );
			printf("\n C/N\t\t\t%f", statusS75CA.CarrierToNoise );
			printf("\n Signal Lock\t\t%d", statusS75CA.SignalLock );
			printf("\n Data Lock\t\t%d", statusS75CA.DataLock );
			printf("\n Uncorrectables\t\t%d", statusS75CA.Uncorrectables );
//			printf("\n Signal Strength: %d, Signal Lock: %d, Data Lock: %d, Uncorrectables: %d", statusS75CA.SignalStrength, statusS75CA.SignalLock, statusS75CA.DataLock,statusS75CA.Uncorrectables);
			printf("\n Noise Estimate\t\t%d", statusS75CA.NEST );
			printf("\n Clock Offset\t\t%d", statusS75CA.Clock_Off );
			printf("\n Frequency Error\t%d", statusS75CA.Freq_Err );
			printf("\n ADC Midpoint Crossing\t%d", statusS75CA.ADC_MID );
			printf("\n ADC Clipping Count\t%d", statusS75CA.ADC_CLIP );
			printf("\n Digital Gain\t\t%d", statusS75CA.Digital_Gain );
			printf("\n Clamping Count\t\t%d\n", statusS75CA.AA_CLAMP );
		}
		if ( k == 0 || k == 4 ) {
			printf("\n PID List\n -------------------\n");
			if ( statusS75CA.PIDList.Control[0] == 3 ) {
				printf( " Entire Transport Stream\n" );
			} else 
			if ( statusS75CA.PIDList.Control[0] == 4 ) {
		...
		if ( k == 0 || k == 5 ) {
			...
			if ( statusS75CA.PIDList.Control[0] == 3 ) {
				printf( " Entire Transport Stream\n" );
				printf( "    %d.%d.%d.%d:%d",
						statusS75CA.PIDDestinations[0].DestinationIP[0],
						statusS75CA.PIDDestinations[0].DestinationIP[1],
						statusS75CA.PIDDestinations[0].DestinationIP[2],
						statusS75CA.PIDDestinations[0].DestinationIP[3],
						statusS75CA.PIDDestinations[0].DestinationUDP    );
			} else 
			if ( statusS75CA.PIDList.Control[0] == 4 ) {
				printf( " Entire Transport Stream Minus NULL Packets\n" );
				printf( "    %d.%d.%d.%d:%d",
				...
				for ( i = 0; i < 32; i++ ) {
					if ( statusS75CA.PIDRoutes[i].PID != NULL_PID ) {
						printf(" 0x%X\n", statusS75CA.PIDRoutes[i].PID);
						line_feed = false;
						for ( j = 0; j < 16; j++ ) {
							if ( statusS75CA.PIDRoutes[i].DestinationMask & ( 0x8000 >> j ) ) {
								printf( "    %d.%d.%d.%d:%d, ",
										statusS75CA.PIDDestinations[j].DestinationIP[0],
										statusS75CA.PIDDestinations[j].DestinationIP[1],
										statusS75CA.PIDDestinations[j].DestinationIP[2],
										statusS75CA.PIDDestinations[j].DestinationIP[3],
										statusS75CA.PIDDestinations[j].DestinationUDP    );
								line_feed = true;
							}
						}
						if ( line_feed )
		...
		if ( k == 0 || k == 6 ) {
			...
			if ( statusS75CA.PIDList.Control[0] == 4 ) {
				...
				for ( j = 0; j < 16; j++ ) {
					if ( statusS75CA.PIDDestinations[j].DestinationUDP ) {
						printf( " %d.%d.%d.%d:%d\n",
								statusS75CA.PIDDestinations[j].DestinationIP[0],
								statusS75CA.PIDDestinations[j].DestinationIP[1],
								statusS75CA.PIDDestinations[j].DestinationIP[2],
						...
						for ( i = 0; i < 32; i++ ) {
							if ( statusS75CA.PIDRoutes[i].DestinationMask & ( 0x8000 >> j ) ) {
								printf( "    0x%x",  statusS75CA.PIDRoutes[i].PID );
							}
						}
						printf( "\n" );
					}
		...
		if ( k == 0 || k == 7 ) {
			printf("\n Program List\n ----------------\n" );	
			for(i=0;i<16;i++) {
				if ( statusS75CA.ProgramList[i] ) {
					printf(" %d\tstatus=%x\n", statusS75CA.ProgramList[i], statusS75CA.ProgramStatus[i]);
				}
			}
			printf( "\n" );
			printf("\n CA Card Status = %x\n", statusS75CA.CardStatus );
		}
...
bool editPIDMappings( S75CA_PIDList *s75caPIDs )
{
	int i, j, k;
	int pid;
	int control;
	...
	do{
		printf( "\n------------PIDS------------\n\n" );
		if ( s75caPIDs->Control[0] == 3 ) {
			printf( "Entire Transport Stream\n" );
		} else
		if ( s75caPIDs->Control[0] == 4 ) {
			printf( "Entire Transport Stream Minus NULL Packets\n" );
		} else {
			for ( i = 0; i < SIZE_PID_LIST; i++ ) {
				if ( s75caPIDs->PID[i] != 0x1fff ) {
					printf( "%d) %d\t(0x%x)\t\t", i, s75caPIDs->PID[i], s75caPIDs->PID[i] );
					switch( s75caPIDs->Control[i] ) {
		...
		if ( s75caPIDs->Control[0] == 3 ) {
			printf( "0) %d.%d.%d.%d:%d, Entire Transport Stream",
					s75caPIDs->Destination[0].DestinationIP[0],
					s75caPIDs->Destination[0].DestinationIP[1],
					s75caPIDs->Destination[0].DestinationIP[2],
					s75caPIDs->Destination[0].DestinationIP[3],
					s75caPIDs->Destination[0].DestinationUDP    );
		} else
		if ( s75caPIDs->Control[0] == 4 ) {
			...
			for ( j = 0; j < NUM_S75CA_DESTINATIONS; j++ ) {
				if ( s75caPIDs->Destination[j].DestinationUDP != 0 ) {
					printf( "%d) %d.%d.%d.%d:%d, ", j,
							s75caPIDs->Destination[j].DestinationIP[0],
							s75caPIDs->Destination[j].DestinationIP[1],
							s75caPIDs->Destination[j].DestinationIP[2],
							s75caPIDs->Destination[j].DestinationIP[3],
							s75caPIDs->Destination[j].DestinationUDP    );
					for ( i = 0; i < SIZE_PID_LIST; i++ ) {
						if ( s75caPIDs->DestinationMask[i] & ( 0x8000 >> j ) ) { 
							printf( "%d  ", s75caPIDs->PID[i] );
						}
					}
					printf( "\n" );
				}
		...
		switch(i){
			...
			case 1:
					...
					if ( found ) {
						s75caPIDs->PID[j] = pid;
						s75caPIDs->Control[j] = control;
						if ( control == 1 )
							s75caPIDs->DestinationMask[j] = 0; // !!! I don't understand this logic
						break;
					}
					j =0;
					while ( (!found) && ( j < SIZE_PID_LIST ) ) {
						if ( s75caPIDs->PID[j] == 0x1fff )
							found = true;
						else
							j++;
					}
			...
			case 2:
					...
					j = 0;
					while ( (!found) && ( j < 16 ) ) {
						if (	( s75caPIDs->Destination[j].DestinationIP[0] == destIP[0] ) &&
								( s75caPIDs->Destination[j].DestinationIP[1] == destIP[1] ) &&
								( s75caPIDs->Destination[j].DestinationIP[2] == destIP[2] ) &&
								( s75caPIDs->Destination[j].DestinationIP[3] == destIP[3] ) &&
								( s75caPIDs->Destination[j].DestinationUDP == destPort ) 		)
							found = true;
						else
							j++;
					}
					...
					if ( found ) {
						memcpy( s75caPIDs->Destination[j].DestinationIP, destIP, 4 );
						s75caPIDs->Destination[j].DestinationUDP = destPort;
					} else {
			...
			case 3:
					...
					if (	( pid_index < 0 ) || ( pid_index >= SIZE_PID_LIST ) ||
						...
						if ( s75caPIDs->PID[pid_index] != 0x1fff ) {
							if ( s75caPIDs->Control[pid_index] != 1 ) {
								if ( s75caPIDs->Destination[destination_index].DestinationUDP != 0 ) {
									s75caPIDs->DestinationMask[pid_index] = s75caPIDs->DestinationMask[pid_index] | (0x8000 >> destination_index);
								} else {
			...
			case 4:
					...
					if ( s75caPIDs->PID[pid_index] < 0x1fff ) {
						s75caPIDs->PID[pid_index] = 0x1fff;
						s75caPIDs->DestinationMask[pid_index] = 0;
					} else {
			...
			case 5:
					...
					if ( 1) {
						memset( s75caPIDs->Destination[destination_index].DestinationIP, 0, 4 );
						s75caPIDs->Destination[destination_index].DestinationUDP = 0;
						mask = ~(0x8000 >> destination_index);
						for ( k = 0; k < SIZE_PID_LIST; k++ ) {
							s75caPIDs->DestinationMask[k] = s75caPIDs->DestinationMask[k] & mask;
						}
						break;
					} else {
    		...
    		case 6:
					...
					if (	( pid_index < 0 ) || ( pid_index >= SIZE_PID_LIST ) ||
						...
						if ( s75caPIDs->PID[pid_index] != 0x1fff ) {
							if ( s75caPIDs->Destination[destination_index].DestinationUDP != 0 ) {
								s75caPIDs->DestinationMask[pid_index] = s75caPIDs->DestinationMask[pid_index] & mask;
							} else {
								printf( "\n Destination not in list!\n" );
							}
						} else {
			...
			case 8:
					...
					for ( j = 0; j < SIZE_PID_LIST; j++ ) {
						s75caPIDs->PID[j] = 0x1fff;
					}
					if ( i == 7 ) 
						s75caPIDs->Control[0] = 3;
					else
						s75caPIDs->Control[0] = 4;
					for ( j = 0; j < 32; j++ ) {
						s75caPIDs->DestinationMask[0] = 0;
					}
					s75caPIDs->DestinationMask[0] = 0x8000;
					for ( j = 0; j < 16; j++ ) {
						memset( s75caPIDs->Destination[j].DestinationIP, 0, 4 );
						s75caPIDs->Destination[j].DestinationUDP = 0;
					}
					memcpy( s75caPIDs->Destination[0].DestinationIP, destIP, 4 );
					s75caPIDs->Destination[0].DestinationUDP = destPort;
					break;
    		case 9: 
					write_table = true;
					break;
...
int userSetPID(S75_Handle hdev)
{
	...
	if ( devType==S75CA ) {
		memset( (void *)&s75caPIDs, 0, sizeof(                 s75caPIDs ) );
		for ( i = 0; i < SIZE_PID_LIST; i++ ) s75caPIDs.PID[i] = 0x1fff;
		result = getPIDList(hdev,(BYTE *)&s75caPIDs);
		if ( editPIDMappings( &s75caPIDs ) ) {
			result = setPIDList(hdev, (BYTE *)&s75caPIDs);
...
int userGetPID(S75_Handle hdev)
{
	...
	if ( devType==S75CA ) {
//		printf("Get PID List returned %X\n",getPIDList(hdev,(BYTE *)&s75caPIDs));
		result = getPIDList(hdev,(BYTE *)&s75caPIDs);
		printf( "\nPIDs & Destinations" );
		printf( "\n---------------------\n" );
		if ( s75caPIDs.Control[0] == 3 ) {
...
int userGetPAT(S75_Handle hdev)
{
	...
	if ( devType==S75CA ) {
		result = getPAT(hdev,(BYTE *)&s75caPAT);
		printf( "\n----Program Association Table----\n" );
		for ( int i = 0; i < 16; i++ ) {
			if ( s75caPAT.ProgramNumber[i] ) {
				printf( " Prog # = %d\tPMT PID = %d\n", s75caPAT.ProgramNumber[i], s75caPAT.PMTPID[i] );
			}
		}
	}
...
bool editPAT( S75CA_PAT *s75caPAT )
{
	int i, j;
	int program;
	int pmt_pid;
	...
	do{
		...
		switch(i){
			...
			case 1:
					...
					j = 0;
					while ( (!found) && ( j < 16 ) ) {
						if ( s75caPAT->ProgramNumber[j] == program )
							found = true;
						else
							j++;
					}
...
int userSetPAT(S75_Handle hdev)
{
	...
	if ( devType==S75CA ) {
		...
		for ( i = 0; i < 16; i++ ) {
			if ( s75caPAT.ProgramNumber[i] ) {
				for ( j = i+1; j < 16; j++ ) {
					if ( s75caPAT.ProgramNumber[j] == s75caPAT.ProgramNumber[i] ) {
						s75caPAT.ProgramNumber[j] = 0;
						s75caPAT.PMTPID[j] = 0;
					}
				}
		...
		if ( editPAT( &s75caPAT ) ) {
//			printf("\n Sending the PAT returned %X\n", setPAT(hdev, (BYTE *)&s75caPAT));
			result = setPAT(hdev, (BYTE *)&s75caPAT);
		}
...
void displayTraps( S75CA_TrapSetting *s75caTraps )
{
	printf( "\n----Traps----\n" );
	printf( " Monitoring Interval is %d\n", s75caTraps->MonitoringInterval );
	printf( " Notification IP:port is %d.%d.%d.%d:%d\n",
			s75caTraps->NotificationIP[0], s75caTraps->NotificationIP[1],
			s75caTraps->NotificationIP[2], s75caTraps->NotificationIP[3],
			s75caTraps->NotificationPort );
	if ( s75caTraps->DataLockMonitorOn ) {
		printf( " Data Lock Monitor is ON\n" );
	} else {
		printf( " Data Lock Monitor is OFF\n" );
	}
	if ( s75caTraps->EthernetTxErrorOn ) {
		printf( " Ethernet TX Error Monitor is ON, " );
	} else {
		printf( " Ethernet TX Error Monitor is OFF, " );
	}
	printf( " Threshold is %d\n", s75caTraps->EthernetTxErrorThreshold );
	if ( s75caTraps->TSErrorOn ) {
		printf( " Transport Stream Uncorrectables Monitor is ON, " );
	} else {
		printf( " Transport Stream Uncorrectables Monitor is OFF, " );
	}
	printf( " Threshold is %d\n", s75caTraps->UncorrectableTSErrorThreshold );
	if ( s75caTraps->VBEROn ) {
		printf( " Viterbi Bit Error Rate Monitor is ON, " );
	} else {
		printf( " Viterbi Bit Error Rate Monitor is OFF, " );
	}
	printf( " Threshold is %d\n", s75caTraps->VBERThreshold );
}
...
{
	WORD devType;
	S75CA_TrapSetting s75caTraps;
	int result = S75_FAILED;

	GetDeviceType(hdev,devType);

	if ( devType==S75CA ) {
		result = getTraps(hdev,(BYTE *)&s75caTraps);
		displayTraps( &s75caTraps );
	}
...
int userSetTraps(S75_Handle hdev)
{
	...
	if ( devType==S75CA ) {
		...
		do {
			...
			switch(i){
				...
				case 1:
						...
						s75caTraps.MonitoringInterval = monitoring_interval;
						break;
				case 2:
						...
						s75caTraps.NotificationIP[0] = (unsigned char)b1;
						s75caTraps.NotificationIP[1] = (unsigned char)b2;
						s75caTraps.NotificationIP[2] = (unsigned char)b3;
						s75caTraps.NotificationIP[3] = (unsigned char)b4;
						s75caTraps.NotificationPort = (WORD)notification_port;
						break;
				case 3:
						printf( "\n Enter 0 (Off) or 1 (On) --> " );
						scanf( "%d", &flag );
						s75caTraps.DataLockMonitorOn = (bool)flag;
						break;
				case 4:
						printf( "\n Enter 0 (Off) or 1 (On) --> " );
						scanf( "%d", &flag );
						...
						s75caTraps.EthernetTxErrorOn = (bool)flag;
						s75caTraps.EthernetTxErrorThreshold = threshold;
						break;
				case 5:
						printf( "\n Enter 0 (Off) or 1 (On) --> " );
						scanf( "%d", &flag );
						printf( "\n Enter Threshold --> " );
						scanf("%d",&threshold);
						s75caTraps.TSErrorOn = (bool)flag;
						s75caTraps.UncorrectableTSErrorThreshold = threshold;
						break;
				case 6:
						printf( "\n Enter 0 (Off) or 1 (On) --> " );
						scanf( "%d", &flag );
						printf( "\n Enter Threshold --> " );
						scanf("%d",&threshold);
						s75caTraps.VBEROn = (bool)flag;
						s75caTraps.VBERThreshold = threshold;
						break;
    			case 7: 
						write_traps = true;
						break;
		...
		if ( write_traps ) {
//			printf("\n Sending the Trap configuration returned %X\n", setTraps(hdev, (BYTE *)&s75caTraps));
			result = setTraps(hdev, (BYTE *)&s75caTraps);
		}
...
int userChangePassword(S75_Handle hdev)
{
	unsigned char key[16] = { 0x8f, 0xb4, 0x3c, 0xd9, 0x65, 0x32, 0x14, 0x56, 0xc5, 0x83, 0xd7, 0x74, 0x81, 0x72, 0xa7, 0xc2 };
	WORD devType;
