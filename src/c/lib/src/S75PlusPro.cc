// S75PlusPro.cpp: implementation of the CS75PlusPro class.
//
//////////////////////////////////////////////////////////////////////
...
CS75PlusPro::CS75PlusPro( bool remote )
{
	m_StatusPacketSize = sizeof(S75PlusProStatus_MSG);
	m_Device_Type = S75PLUSPRO;  // when we instantiate this class, the device type will be enforced on attachment.
	m_StatusWrapper = (StatusPacket *)(new S75PlusProStatusPacket( &m_Status ));
...
int CS75PlusPro::SetPAT(LPBYTE PAT)
{
	int i;

	int last_error_code ;

	S75PlusProPAT_MSG PATMsg;

//	bool ProgramList = false ;

	m_pS75PlusPro_PAT = (S75PlusPro_PAT *) PAT ;
//	m_PAT = new S75PlusPro_PAT( (void *)PAT );
    ...
    for (i = 0; i < 16; i++) {
		...
		PATMsg.PATTableEntry[i].ProgramNumber = htons( m_pS75PlusPro_PAT->ProgramNumber[i] );
		PATMsg.PATTableEntry[i].PMTPID = htons( m_pS75PlusPro_PAT->PMTPID[i] );
	...
	last_error_code = SendPAT( &PATMsg, sizeof( S75PlusProPAT_MSG ), 16 );
...
int CS75PlusPro::GetPAT(LPBYTE PAT)
{
...
//	DWORD xx = 1;                   // Temporary storage variable
	...
	int result;
	bool success=false;
    BYTE Buffer2[sizeof(S75PlusProPAT_MSG)];
	int i;
	int last_error_code ;
	TCHAR temp[16];
	S75PlusProPAT_MSG PATResponseMsg;
	S75PlusProPAT_MSG PATRequest;
//
	...
	digitalsignature( &m_sec, (uint8_t *)(&PATRequest), sizeof( S75PlusProPAT_MSG ) - 20 );
	...
	if ( ( s = socket( AF_INET, SOCK_DGRAM, 17 ) ) == -1 ) {
    	...
    	if ( connect( s, (const struct sockaddr *)(&device_address), sizeof( struct sockaddr_in ) ) != 0){
            ...
            for (i = 0; (!success) && ( i < NUM_RETRIES ); i++) {
                ...
                if ( send( s, (const char *)(&PATRequest), sizeof( S75PlusProPAT_MSG), 0 ) != sizeof( S75PlusProPAT_MSG ) ) {
...
#ifndef LINUX
    		    	        if( (temp1 = recv( s, (char *)Buffer2, sizeof(S75PlusProPAT_MSG), 0 ) ) == SOCKET_ERROR ) {
...
#else
    		    	        if( (temp1 = recv( s, (char *)Buffer2, sizeof(S75PlusProPAT_MSG), 0 ) ) == -1 ) {
					last_error_code = ERR_PAT_RECEIVE;
#endif
	    		    	        ...
	    		    	        if (temp1 == sizeof(S75PlusProPAT_MSG)) {

	            		    		memcpy(&PATResponseMsg, Buffer2, sizeof(S75PlusProPAT_MSG));

									// Go through the PAT and change all the Programs and
    				                // PMT PIDs from network order to host order.
					                for ( i = 0; i < 16; i++) {
   						                m_pS75PlusPro_PAT->ProgramNumber[i] = ntohs(PATResponseMsg.PATTableEntry[i].ProgramNumber);
										m_pS75PlusPro_PAT->PMTPID[i] = ntohs(PATResponseMsg.PATTableEntry[i].PMTPID);
									}
...
int CS75PlusPro::SendPAT( S75PlusProPAT_MSG  *PATMsg, int size, int max_programs )
{
	bool success = false;
	TCHAR  temp[16];
	SOCKET s;
		...
		for (i = 0; i < max_programs; i++) {
	        ...
	        if (PATMsg->PATTableEntry[i].ProgramNumber != 0) {     // Skip Program 0
		        ...
		        for (j = 0; j < max_programs; j++) {
					...
					if ( (((S75PlusProStatusPacket *)m_StatusWrapper)->ProgramListPointer())[j] == PATMsg->PATTableEntry[i].ProgramNumber ) {
						ProgramInList = true;
	                    break;
		            }
...
int CS75PlusPro::GetTraps(LPBYTE TrapParameters)
{
//	WORD opcode;                    // Opcode to send to S75
	int temp1 = 1000;               // Temporary storage variable
//	int count = 0 ;                 // Temporary storage variable
//	DWORD xx = 1;                   // Temporary storage variable
	...
	int result;
	bool success=false;
    BYTE Buffer2[sizeof(S75PlusPro_Event_Trap_MSG)];
	int i;
	int last_error_code ;
	TCHAR temp[16];
	S75PlusPro_Event_Trap_MSG TrapResponse;
	S75PlusPro_Event_Trap_MSG TrapRequest;
	WORD monitor_mask;
//


	m_pS75PlusPro_Traps = (S75PlusPro_TrapSetting *)TrapParameters;
	...
	digitalsignature( &m_sec, (uint8_t *)(&TrapRequest), sizeof( S75PlusPro_Event_Trap_MSG ) - 20 );
	...
	if ( ( s = socket( AF_INET, SOCK_DGRAM, 17 ) ) == -1 ) {
    	...
    	if ( connect( s, (const struct sockaddr *)(&device_address), sizeof( struct sockaddr_in ) ) != 0){
            ...
            for (i = 0; (!success) && ( i < NUM_RETRIES ); i++) {
                ...
                if ( send( s, (const char *)(&TrapRequest), sizeof( S75PlusPro_Event_Trap_MSG), 0 ) != sizeof( S75PlusPro_Event_Trap_MSG ) ) {
...
#ifndef LINUX
    		    	        if( (temp1 = recv( s, (char *)Buffer2, sizeof(S75PlusPro_Event_Trap_MSG), 0 ) ) == SOCKET_ERROR ) {
...
#else
    		    	        if( (temp1 = recv( s, (char *)Buffer2, sizeof(S75PlusPro_Event_Trap_MSG), 0 ) ) == -1 ) {
					last_error_code = ERR_TRAP_RECEIVE;
#endif
	    		    	        ...
	    		    	        if (temp1 == sizeof(S75PlusPro_Event_Trap_MSG)) {

	            		    		memcpy(&TrapResponse, Buffer2, sizeof(S75PlusPro_Event_Trap_MSG));

									// Go through the TrapSettings and change all values from network order to host order.
  						            m_pS75PlusPro_Traps->MonitoringInterval = ntohs(TrapResponse.MonitoringInterval);
  						            memcpy( m_pS75PlusPro_Traps->NotificationIP, TrapResponse.NotificationIPAddress, 4 );
  						            m_pS75PlusPro_Traps->NotificationPort = ntohs(TrapResponse.NotificationPort);
									monitor_mask = ntohs(TrapResponse.Mask);
									m_pS75PlusPro_Traps->DataLockMonitorOn = monitor_mask & 0x0001 ? 1 : 0;
									m_pS75PlusPro_Traps->EthernetTxErrorOn = monitor_mask & 0x0002 ? 1 : 0;
									m_pS75PlusPro_Traps->TSErrorOn = monitor_mask & 0x0004 ? 1 : 0;
									m_pS75PlusPro_Traps->VBEROn = monitor_mask & 0x0008 ? 1 : 0;
  						            m_pS75PlusPro_Traps->VBERThreshold = ntohs(TrapResponse.VBERThreshold);
  						            m_pS75PlusPro_Traps->UncorrectableTSErrorThreshold = ntohl(*((unsigned long *)(&(TrapResponse.UncorrectableTSErrorThreshold[0]))));
  						            m_pS75PlusPro_Traps->EthernetTxErrorThreshold = ntohs(TrapResponse.EthernetTxErrorThreshold);
...
int CS75PlusPro::SetTraps(LPBYTE TrapParameters)
{
	bool			configurationSent;             // New configuration sent to S75?
	bool			configurationApplied=false;          // New configuration applied to S75?
	BYTE			Buffer[sizeof(S75PlusPro_Event_Trap_MSG)];                    // Buffer to hold data to send to S75
	S75PlusPro_Event_Trap_MSG *	pTrapSettings;
	TCHAR			temp[16];                      // Temporary storage variable
	int			LastError;
	int			i;


	m_pS75PlusPro_TrapSetting  = (S75_TrapSetting_JLEO *) TrapParameters ;		// Type Cast the parameters Passed into the structure 
	pTrapSettings = (S75PlusPro_Event_Trap_MSG *) Buffer;					// Initialize the structure pointer to the Buffer
	...
	memset( pTrapSettings, 0, sizeof( S75PlusPro_Event_Trap_MSG ) );

    pTrapSettings->Opcode = htons(0x0002);

	pTrapSettings->MonitoringInterval = htons( (unsigned short)( m_pS75PlusPro_TrapSetting->MonitoringInterval  ) );
	memcpy( pTrapSettings->NotificationIPAddress, m_pS75PlusPro_TrapSetting->NotificationIP, 4 );
	pTrapSettings->NotificationPort = htons( (unsigned short)( m_pS75PlusPro_TrapSetting->NotificationPort  ) );
	pTrapSettings->DataLockMonitorOn = htons( (unsigned short)( m_pS75PlusPro_TrapSetting->DataLockMonitorOn  ) );
	pTrapSettings->VBERThreshold = htons( (unsigned short)( m_pS75PlusPro_TrapSetting->VBERThreshold  ) );
	*((unsigned long *)(pTrapSettings->UncorrectableTSErrorThreshold)) = htonl( (unsigned long)( m_pS75PlusPro_TrapSetting->UncorrectableTSErrorThreshold  ) );
	pTrapSettings->EthernetTxErrorThreshold = htons( (unsigned short)( m_pS75PlusPro_TrapSetting->EthernetTxErrorThreshold  ) );
	pTrapSettings->Mask = 0;
	if ( m_pS75PlusPro_TrapSetting->DataLockMonitorOn ) {
		pTrapSettings->Mask += 1;
	}
	if ( m_pS75PlusPro_TrapSetting->VBEROn ) {
		pTrapSettings->Mask += 2;
	}
	if ( m_pS75PlusPro_TrapSetting->TSErrorOn ) {
		pTrapSettings->Mask += 4;
	}
	if ( m_pS75PlusPro_TrapSetting->EthernetTxErrorOn ) {
...
//	memset( (void *)(&m_sec), 0, sizeof( m_sec ) );
	...
	digitalsignature( &m_sec, (uint8_t *)pTrapSettings, sizeof( S75PlusPro_Event_Trap_MSG ) - 20 );
...
//	}
	...
	for (i = 0; i < NUM_RETRIES; i++) {
        configurationSent = ((LastError = SendWaitForConfirmation( temp, TRAP_CONFIG_PORT, Buffer, sizeof(S75PlusPro_Event_Trap_MSG))) == ERR_SUCCESS);
...
bool CS75PlusPro::ParseStatusPacket()
{
	BYTE temp[4];
	...
	if( ((*(((S75PlusStatusPacket *)m_StatusWrapper)->DSPFirmWarePointer())) & 0xFF00) != 0x6800 )
		 return false ;		

	if(m_Device_Type != S75PLUSPRO) return false;
...
int CS75PlusPro::PollStatus( StatusPacket *status_packet_wrapper, int timeout )
	        	        ...
	        	        if ( result != 0 ) {
    		    	        ...
    		    	        if( (temp1 = recv( s, (char *)(status_packet_wrapper->data()), sizeof(S75PlusStatus_MSG), 0 ) ) == -1 ) {
				    		    ...
				    		    if (temp1 == sizeof(S75PlusStatus_MSG)) {
									...
									if( firmware_version != (short)0x6800 ) {		
										success = false;
										last_error_code = ERR_WRONG_RECEIVER;		
									}

									if(m_Device_Type != S75PLUSPRO) {
										success = false;
...
int CS75PlusPro::GetStatus(void *Container)
{
	...
	if(m_Device_Type != S75PLUSPRO) return ERR_DEVICE_TYPE_MISMATCH ;
