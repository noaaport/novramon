// Free implementation of the GOST R 34.11-94 hash algorithm

/*
 *  gosthash.c 
 *  21 Apr 1998  Markku-Juhani O. Saarinen <mjos@iki.fi>
 * 
...
#define GOST_ENCRYPT_ROUND(k1, k2) t = (k1) + r; l ^= gost_sbox_1[t & 0xff] ^ gost_sbox_2[(t >> 8) & 0xff] ^ gost_sbox_3[(t >> 16) & 0xff] ^ gost_sbox_4[t >> 24]; t = (k2) + l; r ^= gost_sbox_1[t & 0xff] ^ gost_sbox_2[(t >> 8) & 0xff] ^ gost_sbox_3[(t >> 16) & 0xff] ^ gost_sbox_4[t >> 24]; 

/* encrypt a block with the given key */

#define GOST_ENCRYPT(key) GOST_ENCRYPT_ROUND(key[0], key[1]) GOST_ENCRYPT_ROUND(key[2], key[3]) GOST_ENCRYPT_ROUND(key[4], key[5]) GOST_ENCRYPT_ROUND(key[6], key[7]) GOST_ENCRYPT_ROUND(key[0], key[1]) GOST_ENCRYPT_ROUND(key[2], key[3]) GOST_ENCRYPT_ROUND(key[4], key[5]) GOST_ENCRYPT_ROUND(key[6], key[7]) GOST_ENCRYPT_ROUND(key[0], key[1]) GOST_ENCRYPT_ROUND(key[2], key[3]) GOST_ENCRYPT_ROUND(key[4], key[5]) GOST_ENCRYPT_ROUND(key[6], key[7]) GOST_ENCRYPT_ROUND(key[7], key[6]) GOST_ENCRYPT_ROUND(key[5], key[4]) GOST_ENCRYPT_ROUND(key[3], key[2]) GOST_ENCRYPT_ROUND(key[1], key[0]) t = r; r = l; l = t;
