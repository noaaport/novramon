...
A75V::A75V( unsigned char *buffer )
{
	initAttributes();
...
//string A75V::macFromStatus( unsigned char *buffer )
//{
//	char mac_str[20];
//
...
bool A75V::checkStatusPacket( string ip, unsigned char *buffer )
{
...
bool A75V::connected( int timeout )
{
...
bool A75V::setFixedParameters( unsigned char *buffer )
{
	char temp[20];

	if ( buffer[32] == ID_A75V ) {

		currentParameters[DEVICE_TYPE]->setM_value( string( itoa( R_A75V, temp, 10 ) ) );
		...
		currentParameters[SIZE_PID_LIST]->setM_value( string( itoa( NUM_PIDS_A75V ,temp, 10 ) ) );
...
string A75V::fixedParamFromStatus( ReceiverParameter p, unsigned char *buffer )
{
	char temp[100];
	string return_str = "";
...
string A75V::dynamicParamFromStatus( ReceiverParameter p, unsigned char *buffer, int index )
{
