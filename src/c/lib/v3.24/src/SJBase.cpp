...
typedef struct SJPIDRoute {
	WORD PID;
	WORD DestinationMask;
} SJPIDRoute;


typedef struct SJPIDDestination {
	BYTE DestinationIP[4];
	WORD DestinationUDP;
} SJPIDDestination;


typedef struct SJPATEntry {
	WORD ProgramNumber;
	WORD PMTPID;
} SJPATEntry;


typedef struct SJFKEntry {
	WORD PID;
	BYTE key[8];
} SJFKEntry;


typedef struct SJIPRemapRule {
...
typedef struct SJStatusRequestMSG {
	BYTE Message[6];	 
} SJStatusRequestMSG;


typedef struct SJPIDMSG {
...
typedef struct SJPATMSG {

	WORD Opcode;
	SJPATEntry PATTableEntry[NUMBER_OF_DESTINATIONS];
	WORD  Random;				// 
	BYTE  Signature[20];		// HMAC-SHA1
...
typedef struct SJCAProgramStatusMSG {

	WORD Opcode;
	WORD ProgramList[NUMBER_OF_PROGRAMS];
	WORD ProgramStatus[NUMBER_OF_PROGRAMS];

} SJCAProgramStatusMSG;
	

typedef struct SJCAProgramMSG {
...
typedef struct SJCAMWatchdogMSG {
...
typedef struct SJFKMSG {

	WORD Opcode;		
	SJFKEntry	FK[NUMBER_OF_FIXED_KEYS];
	WORD Random;		
	BYTE Signature[20];				
...
typedef struct SJIPRemapMSG {

    WORD			Opcode;
	WORD            enable;
	SJIPRemapRule	remapping_rule[NUMBER_OF_IP_REMAP_RULES];
	WORD			Random;				// 
	BYTE			Signature[20];		// HMAC-SHA1
...
typedef struct SJSetPasswordMSG
{
...
//	TCHAR temp[16];
	...
	try {
		...
		digitalsignature( &m_sec, (uint8_t *)(&pidRequest), sizeof( SJPIDMSG ) - 20 );
        ...
        for ( i = 0; (!success) && ( i < 4 ); i++) {
            ...
            if ( send( login_socket, (const char *)(&pidRequest), sizeof( SJPIDMSG), 0 ) != sizeof( SJPIDMSG ) ) {
...
#ifdef WINDOWS
  		    	        if( (temp1 = recv( login_socket, (char *)Buffer2, sizeof(SJPIDMSG), 0 ) ) == SOCKET_ERROR ) {
...
#else
   		    	        if( (temp1 = recv( login_socket, (char *)Buffer2, sizeof(SJPIDMSG), 0 ) ) == -1 ) {
							last_error_code = N_ERROR_PID_RECEIVE;
#endif
...
//	unsigned short program_number;
	...
	if ( dirty ) {
		...
		try {
			...
			memset( (void *)&patConfig, 0, sizeof( SJPATMSG ) );
			...
			digitalsignature( &m_sec, (uint8_t *)(&patConfig), sizeof( SJPATMSG ) - 20 );
...
//	unsigned short program_number;
	...
	if ( dirtyFlags[CA_PROGRAM_NUMBER] ) {
		...
		try {
			...
			SJCAProgramMSG caConfig;

			memset( (void *)&caConfig, 0, sizeof( SJCAProgramMSG ) );
			...
			digitalsignature( &m_sec, (uint8_t *)(&caConfig), sizeof( SJCAProgramMSG ) - 20 );
...
//	bool dirty = false;
	...
	if ( dirtyFlags[CAM_WATCHDOG_INTERVAL] ) {
		...
		try {
			...
			SJCAMWatchdogMSG CAMWatchdogConfig;

			memset( (void *)&CAMWatchdogConfig, 0, sizeof( SJCAMWatchdogMSG ) );
			...
			digitalsignature( &m_sec, (uint8_t *)(&CAMWatchdogConfig), sizeof( SJCAMWatchdogMSG ) - 20 );
...
//	char temp[10];
	...
	if ( dirty ) {
		...
		try {
			...
			memset( (void *)&pidConfig, 0, sizeof( SJPIDMSG ) );
...
//			pidConfig.DestinationPort = htons( pendingParameters[ STREAM_DESTINATION_PORT ]->asShort() );
			...
			digitalsignature( &m_sec, (uint8_t *)(&pidConfig), sizeof( SJPIDMSG ) - 20 );
...
bool SJBase::applyFixedKeys()
{
	...
	if ( dirty ) {
		...
		try {
			...
			memset( (void *)&fkConfig, 0, sizeof( SJFKMSG ) );
			...
			digitalsignature( &m_sec, (uint8_t *)(&fkConfig), sizeof( SJFKMSG ) - 20 );
...
bool SJBase::applyIPRemapRules()
{
	...
	if ( dirty ) {
		...
		try {
			...
			SJIPRemapMSG ipRemapConfig;

			memset( (void *)&ipRemapConfig, 0, sizeof( SJIPRemapMSG ) );
			...
			digitalsignature( &m_sec, (uint8_t *)(&ipRemapConfig), sizeof( SJIPRemapMSG ) - 20 );
...
bool SJBase::readPIDTable()
{
	...
	SJPIDMSG *pidReply = (SJPIDMSG *)receive_buffer;
	char temp[20];
	unsigned short pid;
//	unsigned short mask;
	...
	if ( getConfig( I_PID_PORT, (unsigned char *)&pidRequest, receive_buffer, sizeof( pidRequest ), sizeof( SJPIDMSG ) ) ) {
...
bool SJBase::readPATTable()
{
	...
	int i;
	unsigned char receive_buffer[1500];
	SJPATMSG *patReply = (SJPATMSG *)receive_buffer;
	char temp[20];
//	unsigned short pid;
	...
	if ( getConfig( I_PAT_CONFIG_PORT, (unsigned char *)&patRequest, receive_buffer, sizeof( patRequest ), sizeof( SJPATMSG ) ) ) {
...
{
	bool success = false;
    SJCAProgramMSG CARequest;
	int i;
	unsigned char receive_buffer[1500];
	SJCAProgramStatusMSG *CAReply = (SJCAProgramStatusMSG *)receive_buffer;
	char temp[20];
//	unsigned short program_number;
	...
	if ( getConfig( I_PROGRAM_CONFIG_PORT, (unsigned char *)&CARequest, receive_buffer, sizeof( CARequest ), sizeof( SJCAProgramStatusMSG ) ) ) {
...
{
	bool success = false;
    SJCAMWatchdogMSG CAMWatchdogRequest;
//	int i;
	unsigned char receive_buffer[1500];
	SJCAMWatchdogMSG *CAMWatchdogReply = (SJCAMWatchdogMSG *)receive_buffer;
	char temp[20];
	...
	if ( getConfig( I_CAM_WATCHDOG_PORT, (unsigned char *)&CAMWatchdogRequest, receive_buffer, sizeof( CAMWatchdogRequest ), sizeof( SJCAMWatchdogMSG ) ) ) {
...
bool SJBase::readFixedKeys()
{
	...
	int i;
	unsigned char receive_buffer[1500];
	SJFKMSG *FKReply = (SJFKMSG *)receive_buffer;
	char temp[20];
	unsigned short pid;
	char key_str[17];
	...
	if ( getConfig( I_FK_CONFIG_PORT, (unsigned char *)&FKRequest, receive_buffer, sizeof( FKRequest ), sizeof( SJFKMSG ) ) ) {
...
{
	bool success = false;
    SJIPRemapMSG IPRemapRequest;
	int i;
	unsigned char receive_buffer[1500];
	SJIPRemapMSG *IPRemapReply = (SJIPRemapMSG *)receive_buffer;
	char temp[20];
	...
	if ( getConfig( I_IP_REMAP_CONFIG_PORT, (unsigned char *)&IPRemapRequest, receive_buffer, sizeof( IPRemapRequest ), sizeof( SJIPRemapMSG ) ) ) {
...
{
	SJSetPasswordMSG password_message;
	int i;
	string mac;
	int mac_byte[6];
	char junk[2];
	...
	digitalsignature(&m_sec, (unsigned char *)(&password_message), sizeof( SJSetPasswordMSG ) - 20 );
