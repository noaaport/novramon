...
typedef struct SJ300PIDRoute {
	WORD PID;
	WORD DestinationMask;
} SJ300PIDRoute;


typedef struct SJ300PIDDestination {
	BYTE DestinationIP[4];
	WORD DestinationUDP;
} SJ300PIDDestination;


typedef struct SJ300StatusMSG {
	...
	BYTE			DemodConfig;	// NEW			// bit 0 - auto symbol rate flag
													...
													// bit 3 - ISI Enable
	BYTE			GoldCode[4]; // NEW
	unsigned short	InputStreamIdentifier;	
	unsigned short	InputStreamIdentifierMask;
	unsigned short	TargetMODCOD;
...
typedef struct SJ300NetConfigMSG {
...
typedef struct SJ300RFConfigMSG {
	...
	WORD  GoldCode[2];
	WORD  InputStreamIdentifier;
	WORD  InputStreamIdentifierMask;
	WORD  TargetMODCOD;
	WORD  Random;				   // 
	BYTE  Signature[20];		   // HMAC-SHA1
...
string SJ300Base::fixedParamFromStatus( ReceiverParameter p, unsigned char *buffer )
{
	char temp[100];
	string return_str = "";
	SJ300StatusMSG *status = (SJ300StatusMSG *)buffer;
...
//											break;
	...
	case ISI_ENABLE						:	return_str = string( itoa( ( status->DemodConfig & 0x8 ) >> 3, temp, 10 ) );
											break;
	...
	case INPUT_STREAM_IDENTIFIER		:	return_str = string ( itoa( ntohs( status->InputStreamIdentifier ) & 0xff, temp, 10 ) );
											break;

	case INPUT_STREAM_IDENTIFIER_MASK	:	return_str = string ( itoa( ntohs( status->InputStreamIdentifierMask ) & 0xff, temp, 10 ) );
											break;

	case TARGET_MODCOD					:	return_str = string ( itoa( ntohs( status->TargetMODCOD ), temp, 10 ) );
											break;
...
bool SJ300Base::pollStatus( unsigned char *packet, int timeout )
{
    ...
    for ( int i = 0; (!success) && ( i < 4 ); i++ ) {
		...
		try {
			...
			if ( recv_len != sizeof( SJ300StatusMSG ) ) throw 5;

			sprintf( temp, "%02x-%02x-%02x-%02x-%02x-%02x",
					 ((SJ300StatusMSG *)packet)->ReceiverMAC[0],
					 ((SJ300StatusMSG *)packet)->ReceiverMAC[1],
					 ((SJ300StatusMSG *)packet)->ReceiverMAC[2],
					 ((SJ300StatusMSG *)packet)->ReceiverMAC[3],
					 ((SJ300StatusMSG *)packet)->ReceiverMAC[4],
					 ((SJ300StatusMSG *)packet)->ReceiverMAC[5] );

			if ( !matches( RECEIVER_MAC, string( temp ) ) ) throw 6;

			if ( ((SJ300StatusMSG *)packet)->DeviceType != typeIDCode() ) throw 7;
...
//			rfConfig.ViterbiRate = 0x08;
			...
			if ( getParameter( ISI_ENABLE, PENDING ).asFlag() ) {
				rfConfig.DemodConfig = rfConfig.DemodConfig | 0x8;
			}
			...
			rfConfig.InputStreamIdentifier = htons( getParameter( INPUT_STREAM_IDENTIFIER, PENDING ).asShort() );
			rfConfig.InputStreamIdentifierMask = htons( getParameter( INPUT_STREAM_IDENTIFIER_MASK, PENDING ).asShort() );

			rfConfig.TargetMODCOD = htons( getParameter( TARGET_MODCOD, PENDING ).asShort() );

			srand( time(NULL) );
			rfConfig.Random = rand();

			digitalsignature( &m_sec, (uint8_t *)(&rfConfig), sizeof( SJ300RFConfigMSG ) - 20 );
...
//	char temp_str[20];
	...
	if ( dirty ) {
		...
		try {
			...
			digitalsignature( &m_sec, (uint8_t *)(&netConfig), sizeof( SJ300NetConfigMSG ) - 20 );
...
bool SJ300Base::reset()
{
	...
	digitalsignature( &m_sec, (uint8_t *)(&resetMsg), sizeof( SJ300ResetMSG ) - 20 );

	memcpy( resetMsg.Signature, m_sec.digest, 20 );

	return sendConfig( I_RESET_PORT, (unsigned char *)&resetMsg, sizeof( SJ300ResetMSG ) );
}
