...
typedef struct S200VCAStatusRequestMSG {
	BYTE Message[6];	 
} S200VCAStatusRequestMSG;


typedef struct S200VCAStatusMsg {
...
typedef struct S200VCADefaultConfigMSG
{
...
} S200VCADefaultConfigMSG;


typedef struct S200VCANetConfigMSG {
...
typedef struct S200VCARFConfigMSG {
...
typedef struct S200VCAResetMSG {
...
typedef struct S200VCASetPasswordMSG {

	unsigned char newPassword[16];
	WORD Random;		
	BYTE Signature[20];				

} S200VCASetPasswordMSG;


typedef struct S200VCAVProgramEntry {

	unsigned short programNumber;
	unsigned char  destinationIP[4];
	unsigned short destinationUDP;

} S200VCAVProgramEntry;



typedef struct S200VCAVProgramListMSG {

	WORD Opcode;
	unsigned short mode;
	unsigned char destinationIP[4];
	unsigned short destinationUDP;
	S200VCAVProgramEntry ProgramList[NUMBER_OF_VPROGRAMS];
	WORD Random;		
	BYTE Signature[20];				

} S200VCAVProgramListMSG;


typedef struct S200VCAGetPSIMSG
{
...
typedef struct S200VCAGetProgramInfoMSG {

	WORD Opcode;
	WORD Random;	  
	BYTE Signature[20];				

} S200VCAGetProgramInfoMSG;


typedef struct S200VCALanguageDescriptor {

	unsigned char languageCode[3];
	unsigned char audioType;

} S200VCALanguageDescriptor;


typedef struct S200VCAProgramInfo {
	...
	S200VCALanguageDescriptor langList[NUMBER_OF_AUDIO_PIDS_PER_PROGRAM];

} S200VCAProgramInfo;


typedef struct S200VCAProgramInfoReplyMSG {

	S200VCAProgramInfo Info[NUMBER_OF_VPROGRAMS];

} S200VCAProgramInfoReplyMSG;
...
S200VCA::S200VCA( unsigned char *buffer )
{
	config_ports = new unsigned short[I_NUM_CONFIG_PORTS];
...
int S200VCA::numConfigPorts()
{
	return I_NUM_CONFIG_PORTS;
}


unsigned short S200VCA::configPort( int i )
{
	if ( i < numConfigPorts() ) {
...
bool S200VCA::checkStatusPacket( string ip, unsigned char *buffer )
{
...
bool S200VCA::setFixedParameters( unsigned char *buffer )
{
	char temp[20];
...
bool S200VCA::createIndexedParameters()
{
	int i;
...
string S200VCA::fixedParamFromStatus( ReceiverParameter p, unsigned char *buffer )
{
	char temp[100];
	string return_str = "";
	S200VCAStatusMSG *status = (S200VCAStatusMSG *)buffer;
...
string S200VCA::dynamicParamFromStatus( ReceiverParameter p, unsigned char *buffer, int index )
{
	S200VCAStatusMSG *status = (S200VCAStatusMSG *)buffer;
	char temp[100];
	string return_str = "";
...
bool S200VCA::connected( int timeout )
{
...
bool S200VCA::pollStatus( unsigned char *packet, int timeout )
{
	bool success = false;
    ...
    for ( int i = 0; (!success) && ( i < 4 ); i++ ) {
		...
		try {
			...
			if ( recv_len != sizeof( S200VCAStatusMSG ) ) throw 5;

			sprintf( temp, "%02x-%02x-%02x-%02x-%02x-%02x",
					 ((S200VCAStatusMSG *)packet)->ReceiverMAC[0],
					 ((S200VCAStatusMSG *)packet)->ReceiverMAC[1],
					 ((S200VCAStatusMSG *)packet)->ReceiverMAC[2],
					 ((S200VCAStatusMSG *)packet)->ReceiverMAC[3],
					 ((S200VCAStatusMSG *)packet)->ReceiverMAC[4],
					 ((S200VCAStatusMSG *)packet)->ReceiverMAC[5] );

			if ( !matches( RECEIVER_MAC, string( temp ) ) ) throw 6;

			if ( ((S200VCAStatusMSG *)packet)->DeviceType != typeIDCode() ) throw 7;
//			if ( ((S200VCAStatusMSG *)packet)->DeviceType != ID_S200Base ) throw 7;
...
bool S200VCA::login( string password )
{
...
//	TCHAR temp[16];
	...
	fd_set readfs;
	int result;
	S200VCAVProgramListMSG programRequest;
    BYTE Buffer2[sizeof(S200VCAVProgramListMSG)];
	...
	try {
		...
		digitalsignature( &m_sec, (uint8_t *)(&programRequest), sizeof( S200VCAVProgramListMSG ) - 20 );
        ...
        for ( i = 0; (!success) && ( i < 4 ); i++) {
            ...
            if ( send( login_socket, (const char *)(&programRequest), sizeof( S200VCAVProgramListMSG ), 0 ) != sizeof( S200VCAVProgramListMSG ) ) {
...
#ifdef WINDOWS
  		    	        if( (temp1 = recv( login_socket, (char *)Buffer2, sizeof(S200VCAVProgramListMSG), 0 ) ) == SOCKET_ERROR ) {
...
#else
   		    	        if( (temp1 = recv( login_socket, (char *)Buffer2, sizeof(S200VCAVProgramListMSG), 0 ) ) == -1 ) {
							last_error_code = N_ERROR_PID_RECEIVE;
#endif
    		    	        ...
    		    	        if (temp1 == sizeof(S200VCAVProgramListMSG)) {
...
bool S200VCA::apply( ReceiverParameter ParameterName )
{
...
bool S200VCA::applyVProgramConfig()
{ 
...
//	char temp[10];
	...
	if ( dirty ) {
		...
		try {
			...
			S200VCAVProgramListMSG vprogramConfig;

			memset( (void *)&vprogramConfig, 0, sizeof( S200VCAVProgramListMSG ) );
			...
			digitalsignature( &m_sec, (uint8_t *)(&vprogramConfig), sizeof( S200VCAVProgramListMSG ) - 20 );
...
//			rfConfig.ViterbiRate = 0x08;
			...
			digitalsignature( &m_sec, (uint8_t *)(&rfConfig), sizeof( S200VCARFConfigMSG ) - 20 );
...
bool S200VCA::applyNetworkConfig()
{
...
//	char temp_str[20];
	...
	if ( dirty ) {
		...
		try {
			...
			digitalsignature( &m_sec, (uint8_t *)(&netConfig), sizeof( S200VCANetConfigMSG ) - 20 );
...
bool S200VCA::readCurrentSettings()
{
	bool success = true;
...
bool S200VCA::setIndexedParameter( ReceiverParameter ParameterName, int index, string ParameterValue )
{
	bool success = false;
...
ParameterValue& S200VCA::getIndexedParameter(	ReceiverParameter ParameterName, 
												int index, 
...
ParameterValue& S200VCA::getDblIndexedParameter( ReceiverParameter ParameterName,
												int index1, 
												int index2, 
												ParameterSet parameter_set	)
{
...
void S200VCA::cancelIndexedParameters()
{
	int i;
...
void S200VCA::clearIndexedParameters()
{
	int i;
...
bool S200VCA::reset()
{
	...
	digitalsignature( &m_sec, (uint8_t *)(&resetMsg), sizeof( S200VCAResetMSG ) - 20 );

	memcpy( resetMsg.Signature, m_sec.digest, 20 );

	return sendConfig( I_RESET_PORT, (unsigned char *)&resetMsg, sizeof( S200VCAResetMSG ) );
}


bool S200VCA::factoryReset()
{
	S200VCADefaultConfigMSG factoryResetMsg;
	...
	digitalsignature( &m_sec, (uint8_t *)(&factoryResetMsg), sizeof( S200VCADefaultConfigMSG ) - 20 );

	memcpy( factoryResetMsg.Signature, m_sec.digest, 20 );

	return sendConfig( I_DEFAULT_CONFIG_PORT, (unsigned char *)&factoryResetMsg, sizeof( S200VCADefaultConfigMSG ) );
}



bool S200VCA::changePassword( char *password, unsigned char key[16] )
{
	S200VCASetPasswordMSG password_message;
	int i;
	string mac;
	int mac_byte[6];
	char junk[2];
	...
	digitalsignature(&m_sec, (unsigned char *)(&password_message), sizeof( S200VCASetPasswordMSG ) - 20 );
...
bool S200VCA::readVProgramList()
{
	bool success = false;
    S200VCAVProgramListMSG VProgramListRequest;
	int i;
	unsigned char receive_buffer[1500];
	S200VCAVProgramListMSG *VProgramListReply = (S200VCAVProgramListMSG *)receive_buffer;
	char temp[20];
	...
	if ( getConfig( I_VIDEO_PROGRAM_PORT, (unsigned char *)&VProgramListRequest, receive_buffer, sizeof( VProgramListRequest ), sizeof( S200VCAVProgramListMSG ) ) ) {
...
bool S200VCA::readStreamInfo()
{
	bool success = false;
	S200VCAGetProgramInfoMSG ProgramInfoRequest;
	unsigned char receive_buffer[1500];
	S200VCAProgramInfoReplyMSG *ProgramInfoReply = (S200VCAProgramInfoReplyMSG *)receive_buffer;
	int i;
	int index;
	int audio_index;
	char temp[20];
	...
	if ( getConfig( I_PROGRAM_INFO_PORT, (unsigned char *)&ProgramInfoRequest, receive_buffer, sizeof( ProgramInfoRequest ), sizeof( S200VCAProgramInfoReplyMSG ) ) ) {
...
bool S200VCA::readProgramNames()
{
//	bool success = false;
    S200VCAGetPSIMSG PSIRequest;
//	int i;
	unsigned char receive_buffer[1500];
	S200VCASITable *siTable = (S200VCASITable *)receive_buffer;
