...
typedef struct S75PLUSPIDRoute {
	WORD PID;
	WORD DestinationMask;
} S75PLUSPIDRoute;


typedef struct S75PLUSPIDDestination {
	BYTE DestinationIP[4];
	WORD DestinationUDP;
} S75PLUSPIDDestination;


typedef struct S75PLUSStatusMSG {
	...
	// PID routing tables
    S75PLUSPIDRoute PIDList[NUMBER_OF_PIDS];
	S75PLUSPIDDestination PIDDestinations[NUMBER_OF_DESTINATIONS];
...
typedef struct S75PLUSNetConfigMSG {
...
typedef struct S75PLUSRFConfigMSG {
...
typedef struct S75PLUSResetMSG {
...
string S75PlusBase::fixedParamFromStatus( ReceiverParameter p, unsigned char *buffer )
{
	char temp[100];
	string return_str = "";
	S75PLUSStatusMSG *status = (S75PLUSStatusMSG *)buffer;
...
string S75PlusBase::dynamicParamFromStatus( ReceiverParameter p, unsigned char *buffer, int index )
{
	...
	case SIGNAL_STRENGTH_AS_PERCENTAGE	:	signal_lock = status->DemodulatorStatus & 0x01;
											...
											if ( getParameter( DEVICE_TYPE ).asShort() == R_S75JPLUS ) {
												if ( ss > 30 ) ss = ss * 1.25;
												if ( ss > 100 ) ss = 100;
												if ( ( ss < 8 ) && ( status->DemodulatorStatus & 0x10 ) ) ss = 12;
											}
											sprintf( temp, "%f", ss );
											return_str = string( temp );
											break;
...
bool S75PlusBase::pollStatus( unsigned char *packet, int timeout )
{
    ...
    for ( int i = 0; (!success) && ( i < 4 ); i++ ) {
		...
		try {
			...
			if ( recv_len != sizeof( S75PLUSStatusMSG ) ) throw 5;

			sprintf( temp, "%02x-%02x-%02x-%02x-%02x-%02x",
					 ((S75PLUSStatusMSG *)packet)->ReceiverMAC[0],
					 ((S75PLUSStatusMSG *)packet)->ReceiverMAC[1],
					 ((S75PLUSStatusMSG *)packet)->ReceiverMAC[2],
					 ((S75PLUSStatusMSG *)packet)->ReceiverMAC[3],
					 ((S75PLUSStatusMSG *)packet)->ReceiverMAC[4],
					 ((S75PLUSStatusMSG *)packet)->ReceiverMAC[5] );

			if ( !matches( RECEIVER_MAC, string( temp ) ) ) throw 6;

			if ( ((S75PLUSStatusMSG *)packet)->DeviceType != typeIDCode() ) throw 7;
...
//	unsigned char statusMsg[1500];
	...
	if ( dirty ) {
		...
		try {
			...
			digitalsignature( &m_sec, (uint8_t *)(&rfConfig), sizeof( S75PLUSRFConfigMSG ) - 20 );
...
//			netConfig.IGMP = getParameter( IGMP_ENABLE, PENDING ).asShort();
			...
			digitalsignature( &m_sec, (uint8_t *)(&netConfig), sizeof( S75PLUSNetConfigMSG ) - 20 );
...
bool S75PlusBase::reset()
{
	...
	digitalsignature( &m_sec, (uint8_t *)(&resetMsg), sizeof( S75PLUSResetMSG ) - 20 );

	memcpy( resetMsg.Signature, m_sec.digest, 20 );

	return sendConfig( I_RESET_PORT, (unsigned char *)&resetMsg, sizeof( S75PLUSResetMSG ) );
}
