...
int Receiver::stringToEnum( ReceiverParameter ParameterName, string enumeratedString )
{
	int val = -1;
	int i;
	...
	if ( ( ParameterName == ALL_PARAMETERS ) || ( (  parameterMask[ParameterName] & F_EXISTS ) != 0 ) ) {
		switch ( ParameterName ) {
			...
			case ALL_PARAMETERS	:	for ( i = 0; i < MAX_RECEIVER_PARAM; i++ ) {
										if ( enumToString( ParameterName, i ) == enumeratedString ) {
											val = i;
											break;
										}
									}
			...
			case DEVICE_TYPE	:	for ( i = 0; i < MAX_DEVICE; i++ ) {
										if ( enumToString( ParameterName, i ) == enumeratedString ) {
											val = i;
											break;
										}
			...
			case BAND							:
			case POLARITY	 					:
			case HI_VOLTAGE_SWITCH				:
			case LONG_LINE_COMPENSATION_SWITCH 	:
			case BAND_SWITCHING_TONE_FREQUENCY 	:
			case SIGNAL_LOCK					:
			case DATA_LOCK						:
			case LNB_FAULT						:
			case IGMP_ENABLE					:
			case DVB_SIGNAL_TYPE				:	
			case SPECTRAL_INVERSION_FLAG		:
			case PILOT_SYMBOL_FLAG				:
			case FRAME_LENGTH					:
			case CA_PROGRAM_SCRAMBLED_FLAG		:
			case RAW_FORWARD_FLAG				:	
			case FORWARD_ALL_FLAG				:
			case FORWARD_NULLS_FLAG				:
			case FIXED_KEY_SIGN					:
//			case CIPHER_SABER_ENABLE			:
			case IP_REMAP_ENABLE				:	for ( i = 0; i < 2; i++ ) {
														if ( enumToString( ParameterName, i ) == enumeratedString ) {
															val = i;
															break;
														}
			...
			case DVB_SIGNAL_TYPE_CONTROL		:	for ( i = 0; i < 3; i++ ) {
														if ( enumToString( ParameterName, i ) == enumeratedString ) {
															val = i;
															break;
														}
			...
			case POLARITY_SWITCHING_VOLTAGE	:	if ( enumeratedString == "11-15V" ) { val = 1; break; }
												if ( enumeratedString == "12-20V" ) { val = 0; break; }
												if ( enumeratedString == "13-18V" ) { val = 0; break; }
			...
			case VITERBI_RATE : for ( i = 0; i <= EIGHT_NINTHS; i++ ) {
									if ( enumToString( ParameterName, i ) == enumeratedString ) {
										val = i;
										break;
									}
			...
			case MODCOD			: 
			case TARGET_MODCOD	:	for ( i = 0; i <= QUARTER_BPSK; i++ ) {
										if ( enumToString( ParameterName, i ) == enumeratedString ) {
											val = i;
											break;
										}
			...
			case PROGRAM_STREAM_TYPE	:	if ( enumeratedString == "MPEG-1 Video" ) { val = VIDEO_1; break; }
											if ( enumeratedString == "MPEG-2 Video" ) { val = VIDEO_2; break; }
											if ( enumeratedString == "MPEG-1 Audio" ) { val = AUDIO_3; break; }
											if ( enumeratedString == "MPEG-2 Audio" ) { val = AUDIO_4; break; }
											if ( enumeratedString == "MPEG-2 A/V" ) { val = AUDIO_DOLBY; break; }
											if ( enumeratedString == "ISO/IEC 13818-6 type A, MPE" ) { val = TYPE_A_MPE; break; }
											if ( enumeratedString == "ISO/IEC 13818-6 type D, DSM-CC Sections" ) { val = TYPE_D_MPE; break; }
											if ( enumeratedString == "H.264 Video" ) { val = VIDEO_H264; break; }
					
											break;


			case CAM_STATUS				:	if ( enumeratedString == "Not Inserted" ) { val = 0; break; }
											if ( enumeratedString == "Initializing" ) { val = 1; break; }
											if ( enumeratedString == "Initialized" ) { val = 3; break; }
											if ( enumeratedString == "Ready" ) { val = 7; break; }

											break;


			case CA_PROGRAM_STATUS		:	for ( i = 1; i <= 4; i++ ) {
												if ( enumToString( ParameterName, i ) == enumeratedString ) {
													val = i;
													break;
												}
			...
			case CA_SYSTEM_ID	:	if ( enumeratedString == "Standardized" ) { val = 0; break; }
									if ( enumeratedString == "Canal Plus" ) { val = 0x0100; break; }
									if ( enumeratedString == "France Telecom" ) { val = 0x0500; break; }
									if ( enumeratedString == "Irdeto" ) { val = 0x0600; break; }
									if ( enumeratedString == "Conax" ) { val = 0x0b00; break; }
									if ( enumeratedString == "Philips" ) { val = 0x0d00; break; }
									if ( enumeratedString == "Beta Technik" ) { val = 0x1700; break; }

									break;


			case IP_REMAP_ACTION	:	if ( enumeratedString == "No Rule" )  { val = NO_RULE; break; }
										if ( enumeratedString == "Normal" ) { val = NORMAL; break; }	
										if ( enumeratedString == "Forward" ) { val = FORWARD; break; }
										if ( enumeratedString == "Discard" ) { val = DISCARD; break; }
	...
	if ( ( ParameterName == ALL_PARAMETERS ) || ( (  parameterMask[ParameterName] & F_EXISTS ) != 0 ) ) {
		...
		case ALL_PARAMETERS:	switch ( enumeratedValue ) {
					...
					case SIZE_AUDIO_PID_LIST 			:	return_str = "SIZE_AUDIO_PID_LIST";
												break;
					case MPE 						:	return_str = "MPE";
												break;
					case CODE_DOWNLOAD 				:	return_str = "CODE_DOWNLOAD";
												break;
					case NETWORK_CODE_DOWNLOAD 		:	return_str = "NETWORK_CODE_DOWNLOAD";
												break;
					...
					case GOLD_CODE 					:	return_str = "GOLD_CODE";
												break;
					case ISI_ENABLE			 		:	return_str = "ISI_ENABLE";
												break;
					case INPUT_STREAM_IDENTIFIER	:	return_str = "INPUT_STREAM_IDENTIFIER";
												break;
					case INPUT_STREAM_IDENTIFIER_MASK :	return_str = "INPUT_STREAM_IDENTIFIER_MASK";
												break;
					case TARGET_MODCOD				:	return_str = "TARGET_MODCOD";
												break;
					...
					case STREAM_DESTINATION_PORT 			:	return_str = "STREAM_DESTINATION_PORT";
												break;
//					case CIPHER_SABER_ENABLE			:	return_str = "CIPHER_SABER_ENABLE";
//												break;
					case CIPHER_SABER_ITERATIONS		:	return_str = "CIPHER_SABER_ITERATIONS";
												break;
					case CIPHER_SABER_KEY_FILE				:	return_str = "CIPHER_SABER_KEY_FILE";
												break;
		...
		case DEVICE_TYPE : switch ( enumeratedValue ) {
								...
								case R_S75J			:	return_str = "S75J";
														break;
								case R_S75JPLUS		:	return_str = "S75J+";
														break;
								...
								case R_S200CAPRO	:	return_str = "S200CA Pro";
														break;
								case R_S200CAPROCS	:	return_str = "S200CA-CS";
														break;
		...
		case ISI_ENABLE	:		if ( enumeratedValue ) {
							return_str = "Enabled";
						} else {
							return_str = "Disabled";
						}
		...
		case TARGET_MODCOD	: 
						  switch ( enumeratedValue ) {
							...
							case MODCOD_UNKNOWN			:	return_str = "Unknown";
															break;
...
//		case CIPHER_SABER_ENABLE	:		if ( enumeratedValue ) {
//							return_str = "Enabled";
//						} else {
//							return_str = "Disabled";
//						}
...
int Receiver::loadConfigFromXML( string file_path, bool load_network )
{
	...
	TiXmlNode *xml_cipher_saber=NULL;
...
//@@@JUNE 8, 2009			m_receiver->setParameter( BROADCAST_STATUS_PORT, string( itoa( port, temp, 10 ) ) );
		...
		if ( hasParameter( AUTO_SYMBOL_RATE ) ) {
			if ( xml_signal->ToElement()->Attribute( "AutoSR" ) ) {
				if ( strcmp( xml_signal->ToElement()->Attribute( "AutoSR" ), "ON" ) == 0 ) {
					setParameter( AUTO_SYMBOL_RATE, "1" );
				} else {
					setParameter( AUTO_SYMBOL_RATE, "0" );
				}
			} else {
				setParameter( AUTO_SYMBOL_RATE, "0" );
			}
		...
		if ( hasParameter( GOLD_CODE ) ) {
			if ( xml_signal->ToElement()->Attribute( "GoldCode" ) ) {
		...
		if ( hasParameter( TARGET_MODCOD ) ) {
			if ( xml_signal->ToElement()->Attribute( "MODCOD" ) ) {
				setParameter( TARGET_MODCOD, string( itoa( stringToEnum( TARGET_MODCOD, xml_signal->ToElement()->Attribute( "MODCOD" ) ) , temp, 10 ) ) );
			} else {
				setParameter( TARGET_MODCOD, "0" );
			}
		}

		if ( hasParameter( ISI_ENABLE ) ) {
			if ( xml_signal->ToElement()->Attribute( "ISI" ) ) {
				isi_str = xml_signal->ToElement()->Attribute( "ISI" );
				setParameter( ISI_ENABLE, "1" );
				setParameter( INPUT_STREAM_IDENTIFIER, string( isi_str ) );
				setParameter( INPUT_STREAM_IDENTIFIER_MASK, "255" );
			} else {
				setParameter( ISI_ENABLE, "0" );
				setParameter( INPUT_STREAM_IDENTIFIER, "0" );
				setParameter( INPUT_STREAM_IDENTIFIER_MASK, "0" );
			}
		}
		...
		if ( hasParameter( LO_FREQUENCY ) ) {
			...
			if ( xml_lnb_spec->ToElement()->Attribute( "LOFrequency" ) ) {
		...
		if ( hasParameter( LONG_LINE_COMPENSATION_SWITCH ) ) {
			if ( xml_lnb_control->ToElement()->Attribute( "LongLineCompensation" ) ) {
		...
		if ( hasParameter( BAND_SWITCHING_TONE_FREQUENCY ) ) {
			if ( xml_lnb_spec->ToElement()->Attribute( "HiLowBandTone" ) ) {
...
//		if ( ( hi_voltage_mode_str = xml_lnb_control->ToElement()->Attribute( "HiVoltageMode" ) ) != "" ) {
		...
		if ( hasParameter( SIZE_IP_REMAP_LIST ) ) {
			if ( ( xml_ip_remap_table = xml_content->FirstChild( "IP_REMAP_TABLE" ) ) ) {
				...
				while ( xml_ip_remap_rule ) {
					...
					setIndexedParameter( IP_REMAP_ACTION, i, string( itoa( stringToEnum( IP_REMAP_ACTION, dataStr ), temp, 10 ) ) );
		...
		if ( hasParameter( CIPHER_SABER_ITERATIONS ) ) {
			if ( ( xml_cipher_saber = xml_content->FirstChild( "CIPHER_SABER" ) ) ) {
//				ip_remap_table_read = true;
				dataStr = xml_cipher_saber->ToElement()->Attribute( "Iterations" );
				setParameter( CIPHER_SABER_ITERATIONS, dataStr );
				dirtyFlags[CIPHER_SABER_ITERATIONS] = true;
...
//	TiXmlElement trap_node( "TRAPS" );
	...
	TiXmlElement cs_node( "CIPHER_SABER" );
...
//    UpdateData(true);
	...
	if ( hasParameter( AUTO_SYMBOL_RATE ) ) {
		if ( getParameter( AUTO_SYMBOL_RATE ).asFlag() ) {
			signal_node.SetAttribute( "AutoSR", "ON" );
		} else {
			signal_node.SetAttribute( "AutoSR", "OFF" );
		}
	}
	...
	if ( hasParameter( TARGET_MODCOD ) ) {
		attribute_value = getParameter( TARGET_MODCOD ).asString();
		signal_node.SetAttribute( "MODCOD", attribute_value.c_str() );
	}

	if ( hasParameter( ISI_ENABLE ) ) {
		if ( getParameter( ISI_ENABLE ).asFlag() ) {
			attribute_value = getParameter( INPUT_STREAM_IDENTIFIER ).asString();
			signal_node.SetAttribute( "ISI", attribute_value.c_str() );
		}
	}
...
//	}
	...
	if ( hasParameter( CIPHER_SABER_ITERATIONS ) ) {
		attribute_value = getParameter( CIPHER_SABER_ITERATIONS ).asString();
		cs_node.SetAttribute( "Iterations", attribute_value.c_str() );
	}
	...
	if ( hasParameter( CIPHER_SABER_ITERATIONS ) ) {	
		content_node.InsertEndChild( cs_node );
	}
