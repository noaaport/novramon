...
#include "S200CAPROCS.h"

S200CAPROCS::S200CAPROCS()
{
	initAttributes();
}


S200CAPROCS::S200CAPROCS( unsigned char *buffer ) : S200Base( buffer )
{
	initAttributes();
...
void S200CAPROCS::initAttributes()
{
...
//	parameterMask[ CIPHER_SABER_ENABLE				] = F_EXISTS | F_CONFIG | F_READ | F_WRITE | F_FLAG | F_ENUM;
	parameterMask[ CIPHER_SABER_ITERATIONS			] = F_EXISTS | F_CONFIG | F_READ | F_WRITE | F_SHORT;
	parameterMask[ CIPHER_SABER_KEY_FILE			] = F_EXISTS | F_CONFIG | F_WRITE | F_STRING;
